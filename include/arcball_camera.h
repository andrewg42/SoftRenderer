...
// * view (optional):
//     * The matrix that will be updated with the new view transform. Previous
...
// * rotation_multiplier:
//     * For amplifying the rotation speed. 1.0 means 1-1 mapping between
//     arcball rotation and camera rotation.
...
// * delta_scroll_ticks:
//     * How many scroll wheel ticks passed since the last update (signed
//     number)
// * flags:
//     * For producing a different view matrix depending on your conventions.
void arcball_camera_update(float eye[3], float target[3], float up[3],
                           float view[16], float delta_time_seconds,
                           float zoom_per_tick, float pan_speed,
                           float rotation_multiplier, int screen_width,
                           int screen_height, int x0, int x1, int y0, int y1,
                           int midclick_held, int rclick_held,
                           int delta_scroll_ticks, unsigned int flags);

// Utility for producing a look-to matrix without having to update a camera.
void arcball_camera_look_to(const float eye[3], const float look[3],
                            const float up[3], float view[16],
                            unsigned int flags);
...
void arcball_camera_update(float eye[3], float target[3], float up[3],
                           ...
                           float zoom_per_tick, float pan_speed,
                           float rotation_multiplier, int screen_width,
                           int screen_height, int px_x0, int px_x1, int px_y0,
                           int px_y1, int midclick_held, int rclick_held,
                           int delta_scroll_ticks, unsigned int flags)
{
    // check preconditions
    {
        ...
        float to_target_len =
            sqrtf(to_target[0] * to_target[0] + to_target[1] * to_target[1] +
    ...
    if (rclick_held) {
        ...
        float arcball_radius =
            (float)(screen_width > screen_height ? screen_width
                                                 : screen_height);
        ...
        // http://number-none.com/product/Understanding%20Slerp,%20Then%20Not%20Using%20It/
        {
            ...
            if (c > 0.9995f) {
                ...
                float qw2 = qw - c;
                float q2_len =
                    sqrtf(qx2 * qx2 + qy2 * qy2 + qz2 * qz2 + qw2 * qw2);
                qx2 /= q2_len;
                qy2 /= q2_len;
                qz2 /= q2_len;
                qw2 /= q2_len;
        ...
        float up2_len =
            sqrtf(up2[0] * up2[0] + up2[1] * up2[1] + up2[2] * up2[2]);
        up2[0] /= up2_len;
        up2[1] /= up2_len;
        up2[2] /= up2_len;
...
end_rotate:
    ...
    float eye_zoom[3] = {look[0] * zoom_dist, look[1] * zoom_dist,
                         look[2] * zoom_dist};
                            ...
                            unsigned int flags)
{
    ...
    float look_len =
        sqrtf(look[0] * look[0] + look[1] * look[1] + look[2] * look[2]);
    float up_len = sqrtf(up[0] * up[0] + up[1] * up[1] + up[2] * up[2]);
    ...
    float s[3] = {f[1] * up_norm[2] - f[2] * up_norm[1],
    ...
    if (!(flags & ARCBALL_CAMERA_LEFT_HANDED_BIT)) {
        // in a right-handed coordinate system, the camera's z looks away from
        // the look direction. this gets flipped again later when you multiply
        // by a right-handed projection matrix (notice the last row of
        // gluPerspective, which makes it back into a left-handed system after
    ...
    // t = [s;u;f] * -eye
    float t[3] = {s[0] * -eye[0] + s[1] * -eye[1] + s[2] * -eye[2],
